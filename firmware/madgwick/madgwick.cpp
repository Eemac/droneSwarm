...
struct mgos_imu_madgwick *mgos_imu_madgwick_create(void) {
  ...
  filter = (mgos_imu_madgwick *) calloc(1, sizeof(struct mgos_imu_madgwick));
  if (!filter) {
    return NULL;
  }
  mgos_imu_madgwick_set_params(filter, 200.0f, 0.1f);
...
bool mgos_imu_madgwick_set_params(struct mgos_imu_madgwick *filter, double freq, double beta) {
  if (!filter) {
    return false;
  }
  filter->beta     = beta;
  ...
  filter->prevTime = esp_timer_get_time();
  return true;
}
...
void mgos_imu_madgwick_calc_delta(struct mgos_imu_madgwick *filter) {
  int64_t delta = esp_timer_get_time() - filter->prevTime;
  if(delta < 15000) {
    filter->inv_freq = delta / 1000000.0f;
  }
  filter->prevTime = esp_timer_get_time();
}

static bool mgos_imu_madgwick_updateIMU(struct mgos_imu_madgwick *filter, double gx, double gy, double gz, double ax, double ay, double az) {
  double recipNorm;
  double s0, s1, s2, s3;
  ...
  filter->counter++;
  mgos_imu_madgwick_calc_delta(filter);
  return true;
}

bool mgos_imu_madgwick_update(struct mgos_imu_madgwick *filter, double gx, double gy, double gz, double ax, double ay, double az, double mx, double my, double mz) {
  if (!filter) {
    return false;
  }
  double recipNorm;
...
bool mgos_imu_madgwick_get_quaternion(struct mgos_imu_madgwick *filter, double *q0, double *q1, double *q2, double *q3) {
  if (!filter) {
    return false;
  }
  if (q0) {
...
bool mgos_imu_madgwick_get_angles(struct mgos_imu_madgwick *filter, double *roll, double *pitch, double *yaw) {
  if (!filter) {
    return false;
  }
  if (roll) {
